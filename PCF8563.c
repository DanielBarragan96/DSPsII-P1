...
volatile static uint8_t format_h = 0;
...
uint8_t setTime (uint8_t hours, uint8_t minutes, uint8_t seconds)
{
    ...
    uint8_t val = i2c_writes (PCF8563_SLAVE_ADDRESS, PCF8563_TIME_ADDRESS,
            THREE_BYTE, &buffer[0], PCF_SUBADRESS_SIZE);
    xSemaphoreGive(mutex_time);
    return val;
}
...
uint8_t setDate (uint8_t day, uint8_t month, uint8_t year)
{
    ...
    uint8_t val = i2c_writes (PCF8563_SLAVE_ADDRESS, PCF8563_DATE_ADDRESS,
            TWO_BYTE, &buffer[0], PCF_SUBADRESS_SIZE);
    xSemaphoreGive(mutex_date);
    return val;
}
